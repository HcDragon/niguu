...
import { apiGetRehabSessions, apiGetCognitiveSessions, apiGetCognitiveProgress, getExercises } from '../../lib/api'
...
export default function PatientHub() {
  ...
  const [todaySession, setTodaySession] = useState(undefined)
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      ...
      try {
        const [physSessions, cogSessions, cogProg] = await Promise.all([
          apiGetRehabSessions(user.id, { order: '-session_number', limit: 7 }),
          apiGetCognitiveSessions(user.id, { order: '-played_at', limit: 7 }),
          apiGetCognitiveProgress(user.id),
        ])

        // Find today's session
        const todaySess = physSessions?.find(s => s.session_date === today) ?? null
  ...
  const hasCognitive = exercises.includes('cognitive_rehab')
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* ── CARD 1: PHYSICAL RECOVERY (only if assigned) ── */}
        {hasPhysical && (
          <FadeCard delay={0.1}>
            <GlassCard goldHover>
              <div style={{ padding: '28px' }}>
                ...
                <p style={{
                  ...
                  Joint tracking · Real-time angle analysis
                </p>
        ...
        {!hasPhysical && !hasCognitive && (
          <FadeCard delay={0.1}>
            <GlassCard>
              <div style={{ padding: '28px', textAlign: 'center' }}>
                <MonoChip text="NO EXERCISES ASSIGNED" color="dim" />
                <p style={{
                  fontFamily: '"DM Sans", sans-serif',
                  fontSize: '14px', color: 'rgba(240,237,230,0.5)',
                  margin: '16px 0 0', lineHeight: 1.6,
                }}>
                  Your doctor hasn't assigned any rehabilitation exercises yet.
                  Please contact your doctor for your treatment plan.
                </p>
              </div>
            </GlassCard>
          </FadeCard>
        ...
        {/* ── CARD 2: COGNITIVE TRAINING (only if assigned) ── */}
        {hasCognitive && (
          <FadeCard delay={0.18}>
            <GlassCard goldHover>
              <div style={{ padding: '28px' }}>
      ...
      <FadeCard delay={0.28}>
        <div style={{ marginTop: '44px' }}>
          ...
          <div style={{
            ...
            {/* Physical sparkline (only if assigned) */}
            {hasPhysical && (
              <GlassCard>
            ...
            {/* Cognitive sparkline (only if assigned) */}
            {hasCognitive && (
              <GlassCard>
                <div style={{ padding: '16px' }}>
                  <span style={{
