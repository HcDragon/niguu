...
import { apiGetRehabSessions, apiGetCognitiveSessions } from '../lib/api'
  ...
  const fetchData = async () => {
    try {
      const [rs, cs] = await Promise.all([
        apiGetRehabSessions(userId, { completed: true, order: 'session_number' }),
        apiGetCognitiveSessions(userId, { order: 'played_at' }),
      ])
      setRehabSessions(rs ?? [])
      setCogSessions(cs ?? [])
    } catch (err) {
